...
def marketing_pref_create_receiver(sender, instance, created, *args, **kwargs):
    if created:
        # in coding for entrepreneur 
        #status_code, response_data = Mailchimp().subscribe(instance.email)
        status_code, response_data = Mailchimp().add_email(instance.email)
        if status_code == 200:
            Mailchimp().pending(instance.email)
        print(status_code, response_data)

post_save.connect(marketing_pref_create_receiver, sender=settings.AUTH_USER_MODEL) # sender = MarketingPreference i found it not good after registring it should send a email to confirm this
